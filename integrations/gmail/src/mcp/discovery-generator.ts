...
interface GeneratedTool {
  ...
  category: 'draft' | 'thread' | 'history' | 'settings' | 'message' | 'other';
}
...
function generateSchemaFromParameters(
  ...
  // Add path/query parameters (skip userId since we default to 'me')
  for (const [paramName, param] of Object.entries(parameters)) {
    if ((param.location === 'path' || param.location === 'query') && paramName !== 'userId') {
...
function generateHandler(
  ...
  // Determine Gmail API method to call
  let apiCall = 'gmail';
  ...
  const params: string[] = ["userId: 'me'"];

  if (method.parameters) {
    for (const [paramName, param] of Object.entries(method.parameters)) {
      if (param.location === 'path' && paramName !== 'userId') {
...
function categorizeMethod(method: DiscoveryMethod): GeneratedTool['category'] {
  ...
  if (id.includes('drafts')) return 'draft';
  if (id.includes('threads')) return 'thread';
  if (id.includes('history')) return 'history';
  if (id.includes('settings')) return 'settings';
  if (id.includes('messages')) return 'message';
  return 'other';
}
...
export async function generateToolsFromDiscovery(
  ...
  // Extract all methods from users resource
  const usersResource = (discovery as any).resources?.users;
  if (!usersResource) {
    throw new Error('Users resource not found in discovery document');
  }

  const allMethods = extractMethods(usersResource, 'users', discovery.schemas);

  // Filter for new methods we want to add
  const targetCategories: GeneratedTool['category'][] = ['draft', 'thread', 'history'];
  const newMethods = allMethods.filter(method => {
    const category = categorizeMethod(method);
    return targetCategories.includes(category) && !isAlreadyImplemented(method.id, existingImpl);
  });
...
async function main() {
  ...
  const discoveryPath = path.join(__dirname, '../../gmail-discovery.json');
  ...
  generateToolsFromDiscovery(discoveryPath, implPath)
    .then(result => {
      const output = `// Auto-generated from Gmail Discovery Document
