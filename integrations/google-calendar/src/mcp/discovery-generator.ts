...
interface GeneratedTool {
  ...
  category: 'acl' | 'calendarList' | 'events' | 'settings' | 'colors' | 'channels' | 'other';
}
...
function generateHandler(
  ...
  // Determine Calendar API method to call
  let apiCall = 'calendar';
...
function categorizeMethod(method: DiscoveryMethod): GeneratedTool['category'] {
  ...
  if (id.includes('.acl.')) return 'acl';
  if (id.includes('.calendarlist.')) return 'calendarList';
  if (id.includes('.events.')) return 'events';
  if (id.includes('.settings.')) return 'settings';
  if (id.includes('.colors.')) return 'colors';
  if (id.includes('.channels.')) return 'channels';
  return 'other';
}
...
export async function generateToolsFromDiscovery(
  ...
  // Extract all methods from all resources (Calendar API has multiple top-level resources)
  const allResources = (discovery as any).resources;
  if (!allResources) {
    throw new Error('No resources found in discovery document');
  }

  let allMethods: DiscoveryMethod[] = [];
  for (const [resourceName, resource] of Object.entries(allResources)) {
    allMethods = allMethods.concat(extractMethods(resource, resourceName, discovery.schemas));
  }

  // Filter for new methods we want to add (exclude custom ones we already implemented)
  const excludeMethodNames = ['insert', 'get', 'list', 'update', 'delete', 'quickAdd', 'query']; // Already have custom implementations
  const targetCategories: GeneratedTool['category'][] = [
    'acl',
    'calendarList',
    'events',
    'settings',
...
async function main() {
  ...
  const discoveryPath = path.join(__dirname, '../../calendar-discovery.json');
  ...
  generateToolsFromDiscovery(discoveryPath, implPath)
    .then(result => {
      const output = `// Auto-generated from Google Calendar Discovery Document
