...
export function createStatementMethods(core: Neo4jCore) {
  return {
    ...
    async getStatement(uuid: string, userId: string): Promise<StatementNode | null> {
      const query = `
        MATCH (s:Statement {uuid: $uuid, userId: $userId})
    ...
    async deleteStatements(uuids: string[], userId: string): Promise<void> {
      ...
      await core.runQuery(query, { uuids, userId });
    },
    ...
    async findContradictoryStatements(params: {
      ...
      const query = `
        MATCH (subject:Entity {userId: $userId})<-[:HAS_SUBJECT]-(s:Statement {userId: $userId})
        MATCH (predicate:Entity {userId: $userId})<-[:HAS_PREDICATE]-(s)
    ...
    async updateStatementRecallCount(userId: string, statementsUuids: string[]): Promise<void> {
      const query = `
        MATCH (s:Statement)
      ...
      await core.runQuery(query, { statementUuids: statementsUuids, userId });
    },

    async getEpisodeIdsForStatements(statementUuids: string[]) {
      if (statementUuids.length === 0) {
        ...
        const records = await core.runQuery(cypher, { statementUuids });
