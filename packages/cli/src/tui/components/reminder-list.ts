import {SelectList, Text, Spacer, Container, Loader, matchesKey} from '@mariozechner/pi-tui';
...
function formatSchedule(schedule: string, maxOccurrences: number | null): string {
	...
	if (hour !== undefined) {
		...
		const h12 = h % 12 || 12;
		timeStr = minute > 0 ? `${h12}:${minute.toString().padStart(2,'0')}${ampm}` : `${h12}${ampm}`;
	}

	if (maxOccurrences === 1) return timeStr ? `Once at ${timeStr}` : 'Once';

	let result = '';
	if (freq === 'MINUTELY') result = interval > 1 ? `Every ${interval} min` : 'Every minute';
	else if (freq === 'HOURLY') result = interval > 1 ? `Every ${interval} hours` : 'Hourly';
	else if (freq === 'DAILY') result = days ? days.split(',').map(d => dayNames[d]??d).join(', ') : interval > 1 ? `Every ${interval} days` : 'Daily';
	else if (freq === 'WEEKLY') result = interval > 1 ? `Every ${interval} weeks` : 'Weekly';
...
function formatRelative(dateStr: string): string {
	...
	const future = diff > 0;
	const str = abs < 3600000 ? `${Math.floor(abs/60000)}m` : abs < 86400000 ? `${Math.floor(abs/3600000)}h` : `${Math.floor(abs/86400000)}d`;
...
export class ReminderList implements Component {
	...
	private updateHeader(): void {
		this.headerText.setText(
			chalk.bold.white('Reminders') + '  ' + this.filterLabel() +
	...
	private buildItems() {
		return this.reminders.map(r => {
			const meta: string[] = [formatSchedule(r.schedule, r.maxOccurrences), r.channel];
			if (r.maxOccurrences !== null) meta.push(`${r.occurrenceCount}/${r.maxOccurrences}`);
			if (r.isActive && r.nextRunAt) meta.push(`next: ${formatRelative(r.nextRunAt)}`);
			return {
				value: r.id,
	...
	private rebuildList(): void {
		...
		this.list.onSelectionChange = item => {
			...
			if (this.hasMore && !this.loading && idx >= this.reminders.length - LOAD_MORE_THRESHOLD) {
				this.load(false);
			}
		};
	...
	private load(reset: boolean): void {
		...
		const isActive = this.filter === 'active' ? 'true' as const : this.filter === 'inactive' ? 'false' as const : undefined;
		...
		if (reset) {
			...
			const loaderComp = new Loader(this.tui, s => chalk.cyan(s), s => chalk.dim(s), 'Loading reminders...');
			loaderComp.start();
			...
			fetchReminders(this.baseUrl, this.apiKey, undefined, isActive, PAGE_SIZE)
				.then(result => {
					loaderComp.stop();
					try { this.bodyContainer.removeChild(loaderComp); } catch { /* ignore */ }
				...
				.catch((err: Error) => {
					loaderComp.stop();
					try { this.bodyContainer.removeChild(loaderComp); } catch { /* ignore */ }
					this.bodyContainer.addChild(new Text(chalk.red('Error: ') + chalk.dim(err.message), 1, 0));
					this.loading = false;
					this.onRender();
				});
		} else {
			fetchReminders(this.baseUrl, this.apiKey, this.cursor ?? undefined, isActive, PAGE_SIZE)
				.then(result => {
	...
	handleInput(data: string): void {
		...
		if (matchesKey(data, 'f')) {
			this.filter = this.filter === 'all' ? 'active' : this.filter === 'active' ? 'inactive' : 'all';
			this.load(true);
			return;
		}
